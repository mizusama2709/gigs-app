...
export default async function DashboardPage() {
  ...
  return (
    <main className="max-w-sm sm:max-w-lg lg:max-w-2xl mx-auto p-4 pt-6 pb-8">
      <div className="mb-5">
        <h1 className="text-[24px] font-bold tracking-tight">Hey, {firstName}</h1>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 mb-5 -mx-4 px-4">
        {isFreelancer && (
          <>
            <Link
              href="/profile/edit"
              className="shrink-0 rounded-full bg-surface border border-ink/15 px-3.5 min-h-[40px] flex items-center text-[11.5px] font-semibold"
            >
              Edit profile
            </Link>
            <Link
            ...
            <Link
              href="/gigs/new"
              className="shrink-0 rounded-full bg-accent text-accent-foreground px-3.5 min-h-[40px] flex items-center text-[11.5px] font-semibold"
            >
              List a gig
            </Link>
          </>
      ...
      {isFreelancer && (
        <>
          <h2 className="font-semibold text-[14px] mb-2">My applications</h2>
          <div className="flex flex-col gap-2.5 mb-6">
            {applications?.map((app) => {
              const job = firstOf(app.jobs);
              if (!job) return null;
              return (
                <Card key={app.id} href={`/jobs/${job.id}`} className="p-3 flex justify-between items-center gap-2">
                  <div className="min-w-0">
                    <div className="font-semibold text-[13px] truncate">{job.title}</div>
      ...
      {isClient && (
        <>
          <h2 className="font-semibold text-[14px] mb-2">My posted jobs</h2>
          <div className="flex flex-col gap-2.5 mb-6">
            {postedJobs?.map((job) => (
              <Card key={job.id} href={`/jobs/${job.id}`} className="p-3 flex justify-between items-center gap-2">
                <div className="font-semibold text-[13px] truncate">{job.title}</div>
          ...
          <h2 className="font-semibold text-[14px] mb-2">Booking requests</h2>
          <div className="flex flex-col gap-2.5 mb-6">
            {bookings?.map((booking) => {
              ...
              return (
                <Card key={booking.id} href={`/gigs/${gig.id}`} className="p-3 flex justify-between items-center gap-2">
                  <div className="min-w-0">
                    <div className="font-semibold text-[13px] truncate">{gig.title}</div>
                    {freelancer && (
      ...
      >
        <button
          type="submit"
          className="bg-surface border border-ink/15 rounded-xl p-2 min-h-[44px] w-full text-sm font-semibold shadow-sm"
        >
          Log out
        </button>
      </form>
